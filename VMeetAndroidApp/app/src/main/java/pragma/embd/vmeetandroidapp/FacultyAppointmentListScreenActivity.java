...
public class FacultyAppointmentListScreenActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.facultyappointmentlistscreen);
    ...
    void getFacultyList(){
        ...
        params.put("columns", "Id, username, empid, emailid, phoneno, availability");
        params.put("conditions", "branch = '" + StudentsMainScreenActivity.str_branch.trim() + "' order by Id");
                        ...
                        try {
                            ...
                            // Else display error message
                                ...
                                for (int js = 0; js <= obj.length() - 1; js++) {
                                    ...
                                    for (int m = 0; m <= result.length() - 1; m += 6) {
                                        bean.value1 = result.getString(m);
                                    }

                                    for (int n = 1; n <= result.length() - 1; n += 6) {
                                        bean.value2 = result.getString(n);
                                    }

                                    for (int o = 2; o <= result.length() - 1; o += 6) {
                                        bean.value3 = result.getString(o);
                                    }

                                    for (int p = 3; p <= result.length() - 1; p += 6) {
                                        bean.value4 = result.getString(p);
                                    }

                                    for (int q = 4; q <= result.length() - 1; q += 6) {
                                        bean.value5 = result.getString(q);
                                    }

                                    for (int r = 5; r <= result.length() - 1; r += 6) {
                                    ...
                                    empBean.add(new BeanDetails(bean.value1, bean.value2, bean.value3, bean.value4, bean.value5, bean.value6));
                                }
                                ...
                                if (empBean.size() > 0) {
                                    ...
                                    adapter = new ListviewAdapterFacultyAppointmentsList(
                                            act, list);
